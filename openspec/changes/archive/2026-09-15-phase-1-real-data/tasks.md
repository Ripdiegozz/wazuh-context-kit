...
- [x] 12.1 Assert sparse-checkout **on disk**, not only in argv. **DONE** — `src/fetch/sparse-disk.test.ts` runs the real `createGitRunner()` and the production `cloneRepo()` against a throwaway `file://` origin built in `tmpdir()` with `uploadpack.allowFilter` enabled, then walks the resulting working tree. 2 tests, 11 assertions, no network. Both directions were proven to have teeth by injected regression: dropping the `sparse-checkout set` call leaves only root files and fails the test, and widening the indexer path set to `["plugins", "wcs"]` materialises `plugins/unrelated/` and `plugins/setup/src/main/java/` and also fails it. **Finding:** cone mode always checks out top-level files in addition to the declared directories, so "only the SPEC 1.2 paths" is exactly "the declared subtrees plus root files" — a boundary no argv-level test could have shown. A `platform` clone therefore lands root files only, which is the on-disk explanation for `wazuh-dashboard` resolving a SHA while contributing no plugin entry.
- [x] 12.2 Add a thin `cli.test.ts`. **DONE** — `src/cli.test.ts` spawns the CLI as a real process (exit codes and stderr are the contract, so an in-process call cannot observe them). 8 tests, 22 assertions, 449 ms, no network. Covers `--fixtures` parity against a pinned `payloadHash` plus byte-identical `matrix.json`/`MATRIX.md` across two runs; the fatal exit-2 paths for a schema-invalid `sources.yml`, a missing `sources.yml`, and `git` unresolvable; and argument handling (`--version` → 0, unknown command → 64, unimplemented command → 2). **Finding:** `PATH=""` does not hide git. An empty or unset `PATH` makes libc fall back to a built-in default (`/bin:/usr/bin`), so the first draft of the git-missing test found git anyway, exited 0, and spent 11 s on the network. Pointing `PATH` at a real empty directory is what actually makes git unresolvable; the suite then runs in 449 ms.
