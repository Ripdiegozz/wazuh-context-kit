...
The habit exists for a reason: an estimated forecast cost a maintainer
round-trip twice. Every widening so far, recorded rather than guessed:

| Change | Cold clone | `.cache/` |
| --- | --- | --- |
| Entering the dashboard-shapes widening | 32 s | 245 MB |
| After it | — | 264 MB |
| After adding `.claude` (2026-09-16) | 34 s | 264 MB — unchanged |

The last row is why the habit is worth keeping in both directions: the widening
that looked most expensive on paper cost nothing measurable.
